# 공공데이터 Open API를 활용한 데이터 분석 및 map 서비스
"""
파이썬 : MASH UP Service
편의시설 알리미
"""
...
naver_url = "file://python"  # 비로그인 오픈 API를 사용하기 위해 등록한 웹 서비스 URL
...
def search_place():
    ...
    request = urllib.request.Request(url)
    request.add_header("X-Naver-Client-Id", "pOZrzVf2ULPCGWhdaKMk")
    request.add_header("X-Naver-Client-Secret", "R7H5MMEDEO")
    try:
        response = urllib.request.urlopen(request)
        rescode = response.getcode()
        if (rescode == 200):
